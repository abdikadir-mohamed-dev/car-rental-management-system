...
const MOCK_ACTIVE_BOOKINGS = [
  { _id: 'BKG-1020', customer: { name: 'Mary Wanjiku' }, vehicle: { name: 'Volkswagen Golf' }, dropoffDate: '2026-08-20', dropoffLocation: 'Lavington', status: 'active' },
]
...
function CheckinManagement() {
  ...
  const confirmCheckin = () => {
    setBookings(bookings.map(b => b._id === selectedBooking._id ? { ...b, status: 'completed' } : b))
          ...
          <tbody>
            {filteredBookings.map((booking) => (
              <tr key={booking._id} className="border-b border-slate-100 hover:bg-slate-50">
                ...
                <td className="py-3 px-4 text-slate-600">{booking.dropoffDate}</td>
                <td className="py-3 px-4 text-slate-600">{booking.dropoffLocation || 'N/A'}</td>
                <td className="py-3 px-4">
                  <span className="badge badge-info capitalize">{booking.status}</span>
                </td>
                <td className="py-3 px-4">
                  {booking.status === 'active' && (
                    <button onClick={() => handleCheckin(booking)} className="btn-primary text-sm px-3 py-1">
                      Check-in
                    </button>
                  )}
                </td>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-xl max-w-lg w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                <label className="label">Ending Mileage (km)</label>
                <input
                  type="text"
                  value={mileage}
              ...
              <div className="flex items-center gap-2">
                ...
                />
                <label htmlFor="lateReturn" className="text-sm text-slate-700">Late return</label>
              </div>
            </div>
