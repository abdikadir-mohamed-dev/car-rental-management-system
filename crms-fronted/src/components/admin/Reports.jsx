...
function Reports() {
  ...
  const monthlyRevenue = reports?.revenue?.labels?.map((month, idx) => ({
    month,
    amount: reports.revenue.values[idx] || 0,
  })) || []
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="card p-6">
          ...
          <p className="text-2xl font-bold text-slate-900">
            KES {reports?.revenue?.values?.reduce((a, b) => a + b, 0)?.toLocaleString() || '0'}
          </p>
        ...
        <div className="card p-6">
          ...
          <p className="text-2xl font-bold text-slate-900">{reports?.bookings?.values?.reduce((a, b) => a + b, 0) || 0}</p>
        ...
        <div className="card p-6">
          ...
          <p className="text-2xl font-bold text-slate-900">
            KES {reports?.revenue?.values?.length ? Math.round(reports.revenue.values.reduce((a, b) => a + b, 0) / reports.revenue.values.length) : 0}
          </p>
