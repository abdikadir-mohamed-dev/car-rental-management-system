...
import { fetchTrips } from '../../redux/slices/driverSlice'
...
export default function DriverTripsPage() {
  ...
  const { trips: apiTrips, loading } = useSelector((state) => state.driver)
  ...
  const displayTrips = apiTrips.length > 0 ? apiTrips : trips
  const filtered = tab === 'All' ? displayTrips : displayTrips.filter((t) => t.status === tab);
    ...
    <div className="p-6 space-y-6 overflow-y-auto">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500">{filtered.length} trip{filtered.length !== 1 ? 's' : ''}</p>
        </div>
      </div>
      ...
      <div className="space-y-3">
        {filtered.map((trip) => {
          const StatusIcon = statusIcon[trip.status] || Clock
            ...
            <div key={trip.id} className="bg-white rounded-xl shadow-sm overflow-hidden">
              ...
              >
                <div className="flex items-center gap-4 min-w-0">
                  <div className={`w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 ${statusStyle[trip.status] || 'bg-slate-100 text-slate-600'}`}>
                    <StatusIcon size={18} />
                  </div>
                ...
                <div className="flex items-center gap-4 flex-shrink-0">
                  ...
                  <span className={`px-2.5 py-1 rounded-full text-xs font-medium ${statusStyle[trip.status] || 'bg-slate-100 text-slate-600'}`}>
                    {trip.status}
                  </span>
                  <ChevronRight
                    size={18}
