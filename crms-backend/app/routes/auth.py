...
def register():
    ...
    drivers_license = data.get('driversLicense', '').strip() if data.get('driversLicense') else None
    license_expiry = data.get('licenseExpiry', '').strip() if data.get('licenseExpiry') else None
    ...
    for admin in admins:
        create_notification(admin.id, 'New Customer Registered', f'{user.name} ({user.email}) created an account.')
    for s in staff:
        create_notification(s.id, 'New Customer Registered', f'{user.name} ({user.email}) created an account.')
...
def forgot_password():
    ...
    reset_token = __import__('random').choice('abcdefghijklmnopqrstuvwxyz0123456789') * 8
    user.reset_password_token = reset_token
    user.reset_password_expire = int((datetime.now(timezone.utc).timestamp() + 3600))
    db.session.commit()
    return jsonify({'message': 'Password reset token sent to email', 'resetToken': reset_token}), 200
...
def reset_password(token):
    ...
    if not user or not user.reset_password_expire or user.reset_password_expire < datetime.now(timezone.utc).timestamp():
