...
def update_vehicle(vehicle_id):
    ...
    if 'registrationNumber' in data:
        vehicle.registration_number = data[
            'registrationNumber'
        ]
    ...
    if 'dailyRentalRate' in data:
        vehicle.daily_rental_rate = data[
            'dailyRentalRate'
        ]
    ...
    # Only allow manual availability changes
    # when vehicle is NOT in maintenance.

    elif 'available' in data:

        vehicle.available = bool(
            data['available']
        )

        if vehicle.status != 'maintenance':
            vehicle.is_available = bool(
                data['available']
            )
...
def create_maintenance():
    ...
    if not issue:
        return jsonify({
            'error': 'Maintenance issue is required'
        }), 400
...
def update_maintenance(maintenance_id):
    ...
    maintenance = Maintenance.query.get_or_404(
        maintenance_id
    )
    ...
    if 'issue' in data:
        ...
        if not data['issue']:
            return jsonify({
                'error': 'Maintenance issue cannot be empty'
            }), 400

        maintenance.issue = data['issue']
    ...
    if vehicle:
        ...
        elif maintenance.status == 'Resolved':
...
@bp.route('/maintenance/<int:maintenance_id>', methods=['DELETE'])
@role_required('admin')
def delete_maintenance(maintenance_id):
    ...
    # If the maintenance record is deleted,
    # return the vehicle to available only if
    # it was specifically in maintenance.
...
def update_staff(staff_id):
    ...
    if (
        ...
        if shift:
            ...
            shift = Shift(
                staff_id=staff_id,
                start_time=data['shiftStart'],
                end_time=data['shiftEnd']
            )
...
def update_policies():
    ...
    for key, value in data.items():
        ...
        policy = RentalPolicy.query.filter_by(
            key=key
        ).first()
...
def admin_update_booking(booking_id):
    ...
    if 'pickupLocation' in data:
        booking.pickup_location = data[
            'pickupLocation'
        ]

    if 'returnLocation' in data:
        booking.return_location = data[
            'returnLocation'
        ]
...
def seed_data():
    ...
    if data.get('staff') is not False:
        ...
        if existing_staff == 0:
            ...
            staff_payloads = [
                {
                    'name': 'Staff 1',
                    'email': 'staff1@drivego.com',
                    'phone': '0711000001'
                },
                {
                    'name': 'Staff 2',
                    'email': 'staff2@drivego.com',
                    'phone': '0711000002'
                },
                {
                    'name': 'Staff 3',
                    'email': 'staff3@drivego.com',
                    'phone': '0711000003'
                },
            ]
            ...
            for i, payload in enumerate(
                ...
                result = create_staff_internal(
                    payload,
                    start,
                    end
                )

                created['staff'].append(
                    result['user']
                )

                created['shifts'].append(
                    result['shift']
                )
    ...
    if data.get('drivers') is not False:
        ...
        existing_drivers = User.query.filter_by(
            role='driver'
        ).count()
        ...
        if existing_drivers == 0:
            ...
            for payload in driver_payloads:
                ...
                result = create_driver_internal(
                    payload
                )

                created['drivers'].append(
                    result['user']
                )
    ...
    if data.get('vehicles') is not False:
        ...
        vehicle_list = [
...

            {
                ...
                'model': 'Camry',
                'year': 2022,
                'registration_number': 'KDB 234B',
                'vehicle_type': 'Sedan',
                'transmission': 'Automatic',
                'fuel_type': 'Petrol',
                'seating_capacity': 5,
                'daily_rental_rate': 4500,
                'status': 'available',
                'available': True,
                'is_available': True,
                'location': 'Westlands',
...

            {
                ...
                'model': '3 Series',
                'year': 2023,
                'registration_number': 'KDC 345C',
                'vehicle_type': 'Sedan',
                'transmission': 'Automatic',
                'fuel_type': 'Petrol',
                'seating_capacity': 5,
                'daily_rental_rate': 8500,
                'status': 'available',
                'available': True,
                'is_available': True,
                'location': 'Kilimani',
...

            {
                ...
                'model': 'C-Class',
                'year': 2023,
                'registration_number': 'KDD 456D',
                'vehicle_type': 'Sedan',
                'transmission': 'Automatic',
                'fuel_type': 'Petrol',
                'seating_capacity': 5,
                'daily_rental_rate': 9500,
                'status': 'available',
                'available': True,
                'is_available': True,
                'location': 'Karen',
...

            {
                ...
                'model': 'Evoque',
                'year': 2023,
                'registration_number': 'KDE 567E',
                'vehicle_type': 'SUV',
                'transmission': 'Automatic',
                'fuel_type': 'Petrol',
                'seating_capacity': 5,
                'daily_rental_rate': 12000,
                'status': 'available',
                'available': True,
                'is_available': True,
                'location': 'Nairobi CBD',
...

            {
                ...
                'model': 'Mustang',
                'year': 2023,
                'registration_number': 'KDG 789G',
                'vehicle_type': 'Sports',
                'transmission': 'Automatic',
                'fuel_type': 'Petrol',
                'seating_capacity': 4,
                'daily_rental_rate': 14000,
                'status': 'available',
                'available': True,
                'is_available': True,
                'location': 'Kilimani',
...

            {
                ...
                'model': 'Prado',
                'year': 2022,
                'registration_number': 'KDH 890H',
                'vehicle_type': 'SUV',
                'transmission': 'Automatic',
                'fuel_type': 'Diesel',
                'seating_capacity': 7,
                'daily_rental_rate': 9000,
                'status': 'maintenance',
                'available': False,
                'is_available': False,
                'location': 'Karen',
...

            {
                ...
                'model': 'CR-V',
                'year': 2023,
                'registration_number': 'KDI 901I',
                'vehicle_type': 'SUV',
                'transmission': 'Automatic',
                'fuel_type': 'Petrol',
                'seating_capacity': 5,
                'daily_rental_rate': 6500,
                'status': 'available',
                'available': True,
                'is_available': True,
                'location': 'Nairobi CBD',
...

            {
                ...
                'model': 'X-Trail',
                'year': 2022,
                'registration_number': 'KDJ 012J',
                'vehicle_type': 'SUV',
                'transmission': 'Automatic',
                'fuel_type': 'Petrol',
                'seating_capacity': 7,
                'daily_rental_rate': 7000,
                'status': 'available',
                'available': True,
                'is_available': True,
                'location': 'Westlands',
...

            {
                ...
                'model': 'Golf',
                'year': 2023,
                'registration_number': 'KDK 123K',
                'vehicle_type': 'Hatchback',
                'transmission': 'Automatic',
                'fuel_type': 'Petrol',
                'seating_capacity': 5,
                'daily_rental_rate': 4000,
                'status': 'available',
                'available': True,
                'is_available': True,
                'location': 'Kilimani',
...

            {
                ...
                'model': 'CX-5',
                'year': 2023,
                'registration_number': 'KDL 234L',
                'vehicle_type': 'SUV',
                'transmission': 'Automatic',
                'fuel_type': 'Petrol',
                'seating_capacity': 5,
                'daily_rental_rate': 7500,
                'status': 'available',
                'available': True,
                'is_available': True,
                'location': 'Karen',
...

            {
                ...
                'model': 'Forester',
                'year': 2023,
                'registration_number': 'KDM 345M',
                'vehicle_type': 'SUV',
                'transmission': 'Automatic',
                'fuel_type': 'Petrol',
                'seating_capacity': 5,
                'daily_rental_rate': 7800,
                'status': 'available',
                'available': True,
                'is_available': True,
                'location': 'Nairobi CBD',
...

            {
                ...
                'model': 'Tucson',
                'year': 2023,
                'registration_number': 'KDN 456N',
                'vehicle_type': 'SUV',
                'transmission': 'Automatic',
                'fuel_type': 'Petrol',
                'seating_capacity': 5,
                'daily_rental_rate': 6200,
                'status': 'available',
                'available': True,
                'is_available': True,
                'location': 'Westlands',
...

            {
                ...
                'model': 'Sportage',
                'year': 2023,
                'registration_number': 'KDO 567O',
                'vehicle_type': 'SUV',
                'transmission': 'Automatic',
                'fuel_type': 'Petrol',
                'seating_capacity': 5,
                'daily_rental_rate': 6800,
                'status': 'available',
                'available': True,
                'is_available': True,
                'location': 'Kilimani',
...

            {
                ...
                'model': 'Tahoe',
                'year': 2022,
                'registration_number': 'KDR 890R',
                'vehicle_type': 'SUV',
                'transmission': 'Automatic',
                'fuel_type': 'Petrol',
                'seating_capacity': 8,
                'daily_rental_rate': 11000,
                'status': 'available',
                'available': True,
                'is_available': True,
                'location': 'Nairobi CBD',
